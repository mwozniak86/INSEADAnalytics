...
<br>
1. As such a command `sum(myData != StockReturns[1:20,])` does not make any sense because it compares two different matrices (with different dimensions). To make sense out of it, one would need to set a proper separator (i.e, ',' instead of ';' during CSV file load). Then, few manipulations need to be made with myData variable to makek sure that we compare "apples to apples", i.e. matrices with similar dimensions. To do it, myData needs to set date as new row names (by using `rownames(myData) <- myData[,1]` command) and drop first column (by using `myData <- myData[,-1]` command).
<br>
<br>
<br>
