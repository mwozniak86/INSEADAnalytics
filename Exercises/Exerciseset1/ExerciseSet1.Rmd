...
<br>
Yes. I found an interesting package from above links called Rblpapi that enables to connect R to Bloomberg system.
<br>
<br>
...
<br>
1. This is done in the initial phase where dataset is being loaded. It involves the file: `r file.path("dataSet1.R")`
<br>
2.
<br>
...
<br>
3.
Yes. The script that is responsible for loading data (`r file.path("dataSet1.R")`) would need to be modified to include Yahoo's ticker (i.e. mytickers = c("SPY", "AAPL", "YHOO"))
<br>
<hr>
<hr>
...
1. One can use command `nrow(transposedData)` that gives number of rows: `r nrow(transposedData)` and command `ncol(transposedData)` that gives number of columns: `r ncol(transposedData)`
<br>
2. 
```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE,results='asis',fig.align='center', fig=TRUE}
t_portfolio = apply(transposedData,2,mean)
names(t_portfolio) <- colnames(transposedData)
...
<br>
1. In dataSet1.R file one needs to change startDate variable (line 9) to "2005-01-01"
<br>
2. One would need edit variable `mytickers` in the file `dataSet1.R`, e.g. add YHOO, GOOG, etc.
<br>
<hr>
<hr>
...
```{r eval = FALSE, echo=TRUE, warning=TRUE, message=TRUE}

result <- sum(myData != StockReturns[1:20,])
print(result)

myData + StockReturns[1:40,]

myDataMatrix <- matrix(nrow = 20, ncol = 2)
for (i in 1:2) {
  for (j in 1:20) {
    myDataMatrix[j,i] = myData[j,i]
  }
}

print(myDataMatrix)

myDataMatrix + StockReturns[1:40,] # will result in error as these two matrices have different dimensions

```

<br>
1. As such a command `sum(myData != StockReturns[1:20,])` does not make any sense because it compares two different data structures (with different dimensions). To make sense out of it, one would need to set a proper separator (i.e, ',' instead of ';' during CSV file load). Then, few manipulations need to be made with myData variable to makek sure that we compare "apples to apples", i.e. matrices with similar dimensions. To do it, myData needs to set date as new row names (by using `rownames(myData) <- myData[,1]` command) and drop first column (by using `myData <- myData[,-1]` command). Lastly, one needs to convert myData to matrix (instead of data.frame) for example by using two for loops.
<br>
<br>
2. Once fixes mentioned above are in place `myData + StockReturns[1:40,]` will result in error simply because those two arrays are non-conformable (`Error in myDataMatrix + StockReturns[1:40, ] : non-conformable arrays`)
<br>
<hr>
<hr>
...
```{r eval = TRUE, echo=TRUE, comment=NA, warning=FALSE, message=FALSE,results='asis'}
y <- array(1:100,dim=c(10,10))
write.csv(y, file = "extra_question.csv", row.names = TRUE, col.names = TRUE)
extraData <- read.csv(file = "extra_question.csv", header = TRUE, sep=",")
rownames(extraData) <- extraData[,1]
extraData <- extraData[,-1]
```
<br>
Dimensions: `r dim(extraData)`
<br>
Columns: `r ncol(extraData)`
<br>
Rows: `r nrow(extraData)`
<br>
Sum of all values: `r sum(extraData)`
<br>
